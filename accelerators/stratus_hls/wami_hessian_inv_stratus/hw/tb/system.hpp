...
#ifdef CADENCE
    #include "wami_hessian_inv_wrap.h"
#endif
...
class system_t : public esp_system<DMA_WIDTH, 5000000>
{
  public:
    // ACC instance
#ifdef CADENCE
    wami_hessian_inv_wrapper *acc;
#else
    wami_hessian_inv *acc;
#endif
        ...
        : esp_system<DMA_WIDTH, 5000000>(name)
    {
        // ACC
#ifdef CADENCE
        acc = new wami_hessian_inv_wrapper("wami_hessian_inv_wrapper");
#else
        acc = new wami_hessian_inv("wami_hessian_inv_wrapper");
#endif
        // Binding ACC
        acc->clk(clk);
        acc->rst(acc_rst);
