...
#define SLD_HUANGEMM3S16 0x02c
#define DEV_NAME "sld,huangemm3s16_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s16_len*huangemm3s16_vec;
	// 	out_words_adj = huangemm3s16_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s16_len*huangemm3s16_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s16_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s16_n);
	// out_len = out_words_adj * (huangemm3s16_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3S16, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s16 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3S16_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3S16_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3S16_LOADED_COLS_REG, loaded_cols);
