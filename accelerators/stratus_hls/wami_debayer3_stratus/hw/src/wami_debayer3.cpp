...
#include "wami_debayer3_functions.hpp"

// Processes

void wami_debayer3::load_input()
{
    ...
    for (uint32_t img = 0; img < img_num_img; img++) {
        ...
        uint32_t   length = (img_num_row + 2 * PAD) * (img_num_row + 2 * PAD);
...
void wami_debayer3::store_output()
{
    uint32_t img_num_img          = 0;
    uint32_t img_num_row          = 0;
    uint32_t img_num_col          = 0;
    ...
    for (uint32_t img = 0; img < img_num_img; img++) {
        ...
        for (unsigned i = 0; i < length; i++) {
            ...
            HLS_BREAK_DEP(B0_debayer);
            HLS_PROTO("store-dma-2");
            wait();
            rgb_pixel_t             rgb_pixel_tmp = B0_debayer[i];
            sc_dt::sc_bv<WORD_SIZE> pixel_bv;
            wait();
            pixel_bv.range(47, 32) = rgb_pixel_tmp.r;
            wait();
            pixel_bv.range(31, 16) = rgb_pixel_tmp.g;
            wait();
            pixel_bv.range(15, 0) = rgb_pixel_tmp.b;
...
void wami_debayer3::compute_kernel()
{
    uint32_t img_num_img = 0;
    uint32_t img_num_row = 0;
    uint32_t img_num_col = 0;
    ...
    for (uint16_t n = 0; n < img_num_img; n++) {
        ...
        wami_debayer(img_num_row + 2 * PAD, img_num_col + 2 * PAD);
        wait();
