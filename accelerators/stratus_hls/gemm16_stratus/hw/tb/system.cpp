...
void system_t::config_proc()
{
    ...
    // Compute
    {
    	...
    	ESP_REPORT_TIME(begin_time, "BEGIN - gemm16");
    	...
    	ESP_REPORT_TIME(end_time, "END - gemm16");
...
int system_t::validate()
{
    ...
    gemm16_pv(matrix_inA, matrix_inB, &matrix_out_gold);
    ...
    for (uint32_t d2 = 0; d2 < size_out * batch_size; ++d2)
    {
        if (check_error_threshold(matrix_out->data[d2], matrix_out_gold->data[d2], rel_error))
        {
            ...
            {
                ESP_REPORT_INFO("gemm16[%d] = %lf (%lf) error: %.2lf%%", d2,
