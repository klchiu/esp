...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id      = SUBTRACT_KERN_ID;
        batch        = 0;
        base_addr_0  = 0;                                                                     // use this for output
        base_addr_1  = WAMI_SUBTRACT_IMG_NUM_ROWS * WAMI_SUBTRACT_IMG_NUM_COLS;               // use this for input 1
        base_addr_2  = base_addr_1 + WAMI_SUBTRACT_IMG_NUM_ROWS * WAMI_SUBTRACT_IMG_NUM_COLS; // use this for input 2
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_sub3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_sub3");
...
void system_t::load_memory()
{
    ...
    // Load second input
    mem_base_addr = base_addr_2; // 128*128 + 128*128
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
        ...
        // convert float to fx
        data_fp = FPDATA(golden_warp_img[index]);
        // convert fx to bv
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        // conver fp to float
        sub[index] = (float)data_fp;
        // printf("[dump] : %d\t%f\n", index, sub[i]);
    }
    write_float_matrix_to_file(sub, (srcRows - 2 * PAD), (srcCols - 2 * PAD), "../output/fish_sub_out.txt", "Subtract");
...
int system_t::validate()
{
    ...
    for (uint32_t row = 0; row < srcRows - 2 * PAD; row++) {
        for (uint32_t col = 0; col < srcCols - 2 * PAD; col++) {
            ...
            if (abs(golden_sub[i] - sub[i]) > 0.001) {
                error_count++;
                if (error_count < 5) {

                    ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, sub[i], golden_sub[i]);
                }
            }
            total++;
            // ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, sub[i], golden_sub[i]);
