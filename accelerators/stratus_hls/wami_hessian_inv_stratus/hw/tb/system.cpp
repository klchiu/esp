...
void system_t::config_proc()
{
    ...
    base_addr_1 = 36; // [humu]: use this for now, for input
    ...
    // Config
    {
        conf_info_t config(base_addr, nTestImages, nCols - 2*PAD, nRows - 2*PAD, padding_in, INVERT_GAUSS_JORDAN_KERN_ID, base_addr_1,
                           base_addr_2);
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_hessian_inv");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_hessian_inv");
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < 36; index++) {
        ...
        // conver fp to float
        hess_inv[index] = (float)data_fp;
    }
    write_float_matrix_to_file(hess_inv, 36, 1, "../output/fish_inv_out.txt", "Fish Inv");
...
int system_t::validate()
{
    ...
    for (uint32_t i = 0; i < 36; ++i) {
        if (abs(golden_hess_inv[i] - hess_inv[i]) > 0.001) {
            error_count++;
            ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, hess_inv[i], golden_hess_inv[i]);
        }
        total++;
        ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, hess_inv[i], golden_hess_inv[i]);
    }
