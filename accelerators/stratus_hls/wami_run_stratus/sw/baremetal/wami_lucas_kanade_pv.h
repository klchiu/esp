#ifndef _WAMI_LUCAS_KANADE_PV_H_
#define _WAMI_LUCAS_KANADE_PV_H_
...
// #include "timer.h" // [humu]: comment out for baremetal
...
/*  // [humu]: comment this out for baremetal
                         ...
                         int nCols, fltPixel_t *warped_tmplt)
{
    ...
    /* Allocate and check ------------------------------------------------------ */
    error_img  = aligned_malloc(nRows * nCols * sizeof(fltPixel_t));
    img_dx     = aligned_malloc(nRows * nCols * sizeof(fltPixel_t));
    img_dy     = aligned_malloc(nRows * nCols * sizeof(fltPixel_t));
    nabla_Ix   = aligned_malloc(nRows * nCols * sizeof(fltPixel_t));
    nabla_Iy   = aligned_malloc(nRows * nCols * sizeof(fltPixel_t));
    I_steepest = aligned_malloc(6 * nRows * nCols * sizeof(fltPixel_t));
    sd_delta_p = aligned_malloc(6 * 1 * sizeof(fltPixel_t));
    delta_p    = aligned_malloc(6 * 1 * sizeof(fltPixel_t));
    H          = aligned_malloc(6 * 6 * sizeof(float));
    H_wsp      = aligned_malloc(6 * 6 * sizeof(float));
    H_inv      = aligned_malloc(6 * 6 * sizeof(float));
    ...
    */
    ...
    /*
    // [humu]: comment out the free() for baremetal
        if (error_img)
            free(error_img);
        if (img_dx)
            free(img_dx);
...
float interpolate(float Tlocalx, float Tlocaly, int nCols, int nRows, fltPixel_t *Iin)
{
    ...
    fTlocalx = (float)((int)(Tlocalx));
    fTlocaly = (float)((int)(Tlocaly));
#if 0  
...
#endif /* _WAMI_LUCAS_KANADE_PV_H_ */
