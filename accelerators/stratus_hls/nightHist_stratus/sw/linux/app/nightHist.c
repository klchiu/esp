...
#define DEVNAME "/dev/nightHist_stratus.0"
#define NAME    "nightHist_stratus"
...
static const char usage_str[] = "usage: ./nightHist_stratus.exe coherence infile [nimages] [ncols] [nrows] [-v]\n"
...
struct nightHist_test {
    struct test_info                  info;
    struct nightHist_stratus_access desc;
    char *                            infile;
    bool                              infile_is_raw;
    unsigned                          nimages;
    unsigned                          rows;
...
static inline struct nightHist_test *to_nightHist(struct test_info *info)
{
    return container_of(info, struct nightHist_test, info);
}
...
static void init_buf(struct nightHist_test *t)
{
...
static inline size_t nightHist_size(struct nightHist_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(int);
}

static inline size_t nightHist_size_opt(struct nightHist_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(short);
}

static void nightHist_alloc_buf(struct test_info *info)
{
    struct nightHist_test *t = to_nightHist(info);

    t->hbuf = malloc0_check(nightHist_size_opt(t));
    if (!strcmp(info->cmd, "test")) {
        t->sbuf_in  = malloc0_check(nightHist_size(t));
        t->sbuf_out = malloc0_check(nightHist_size(t));
    }
}

static void nightHist_alloc_contig(struct test_info *info)
{
    struct nightHist_test *t = to_nightHist(info);

    printf("HW buf size: %zu B\n", nightHist_size(t));
    if (contig_alloc(nightHist_size(t), &info->contig) == NULL)
        die_errno(__func__);
}

static void nightHist_init_bufs(struct test_info *info)
{
    struct nightHist_test *t = to_nightHist(info);

    init_buf(t);
    contig_copy_to(info->contig, 0, t->hbuf, nightHist_size_opt(t));
}

static void nightHist_set_access(struct test_info *info)
{
    struct nightHist_test *t = to_nightHist(info);
...
static void nightHist_comp(struct test_info *info)
{
    struct nightHist_test *t      = to_nightHist(info);
    int                      i      = 0;
...
static bool nightHist_diff_ok(struct test_info *info)
{
    struct nightHist_test *t         = to_nightHist(info);
    int                      total_err = 0;
    int                      i;

    contig_copy_from(t->hbuf, info->contig, 0, nightHist_size_opt(t));
...
static struct nightHist_test nightHist_test = {
    .info =
        {
            .name         = NAME,
            .devname      = DEVNAME,
            .alloc_buf    = nightHist_alloc_buf,
            .alloc_contig = nightHist_alloc_contig,
            .init_bufs    = nightHist_init_bufs,
            .set_access   = nightHist_set_access,
            .comp         = nightHist_comp,
            .diff_ok      = nightHist_diff_ok,
            .esp          = &nightHist_test.desc.esp,
            .cm           = NIGHTHIST_STRATUS_IOC_ACCESS,
        },
};
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3 || argc == 5 || argc > 7) {
        ...
        nightHist_test.infile = argv[2];
        printf("\tinfile: %s\n", nightHist_test.infile);

        if (argc == 6 || argc == 7) {
            nightHist_test.nimages = strtol(argv[3], NULL, 10);
            nightHist_test.cols    = strtol(argv[4], NULL, 10);
            nightHist_test.rows    = strtol(argv[5], NULL, 10);
            printf("\tnimages: %u\n", nightHist_test.nimages);
            printf("\tncols: %u\n", nightHist_test.cols);
            printf("\tnrows: %u\n", nightHist_test.rows);
        } else {
            nightHist_test.nimages = DEFAULT_NIMAGES;
            nightHist_test.rows    = DEFAULT_NROWS;
            nightHist_test.cols    = DEFAULT_NCOLS;
        }
        ...
        if (argc == 4 || argc == 7) {
            if ((strcmp(argv[3], "-v") && argc == 4) || (strcmp(argv[6], "-v") && argc == 7)) {
                ...
                nightHist_test.verbose = true;
                printf("\tverbose enabled\n");
            }
        }
        nightHist_test.nbytespp      = DEFAULT_NBYTESPP;
        nightHist_test.swapbytes     = DEFAULT_SWAPBYTES;
        nightHist_test.do_dwt        = DEFAULT_DO_DWT;
        nightHist_test.infile_is_raw = DEFAULT_INFILE_IS_RAW;
        nightHist_test.nbpp_in       = DEFAULT_NBPP_IN;
        nightHist_test.nbpp_out      = DEFAULT_NBPP_OUT;
        printf("\n");
    }

    return test_main(&nightHist_test.info, argv[1], "test");
}
