...
/**
 * Baremetal device driver for NIGHTHISTEQ
 *
...
#define SLD_NIGHTHISTEQ 0x13
#define DEV_NAME        "sld,nightHistEq_stratus"
...
#define NIGHTHISTEQ_BUF_SIZE (ROWS * COLS * 2 * sizeof(pixel))

/* Size of the contiguous chunks for scatter/gather */
#define CHUNK_SHIFT 10
#define CHUNK_SIZE  BIT(CHUNK_SHIFT)
#define NCHUNK                                                                      \
    ((NIGHTHISTEQ_BUF_SIZE % CHUNK_SIZE == 0) ? (NIGHTHISTEQ_BUF_SIZE / CHUNK_SIZE) \
                                              : (NIGHTHISTEQ_BUF_SIZE / CHUNK_SIZE) + 1)

// User defined registers
#define NIGHTHISTEQ_NIMAGES_REG 0x40
#define NIGHTHISTEQ_NROWS_REG   0x44
#define NIGHTHISTEQ_NCOLS_REG   0x48
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_NIGHTHISTEQ, DEV_NAME);
    if (ndev <= 0) {
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            printf("*** CHUNK_SIZE: %d\n", CHUNK_SIZE);
            printf("*** NCHUNK: %d\n", NCHUNK);
            printf("*** NIGHTHISTEQ_BUF_SIZE: %d\n", NIGHTHISTEQ_BUF_SIZE);
            printf("*** IMG_SIZE: %d\n", IMG_SIZE);
            ...
            mem = aligned_malloc(NIGHTHISTEQ_BUF_SIZE);
    ...
    #include "data_480x640.h"
            ...
            iowrite32(dev, NIGHTHISTEQ_NIMAGES_REG, 1);
            iowrite32(dev, NIGHTHISTEQ_NROWS_REG, ROWS);
            iowrite32(dev, NIGHTHISTEQ_NCOLS_REG, COLS);
#if (ROWS == 32 && COLS == 32)
            iowrite32(dev, NIGHTHISTEQ_DO_DWT_REG, 0);
#else
            iowrite32(dev, NIGHTHISTEQ_DO_DWT_REG, 1);
#endif
