...
#
define_hls_module wami_hessian3 ../src/wami_hessian3.cpp
...
foreach dma_width [list 64] {
    ...
    define_sim_config "BEHAV_$ext" "wami_hessian3 BEH" "tb TESTBENCH_$ext" \
        -io_config IOCFG_$ext -argv $DEFAULT_ARGV

	foreach cfg [list BASIC] {
        set cname $cfg\_$ext
        define_hls_config wami_hessian3 $cname -io_config IOCFG_$ext \
            $COMMON_HLS_FLAGS -DHLS_DIRECTIVES_$cfg

		if {$TECH_IS_XILINX == 1} {
		    define_sim_config "$cname\_V" "wami_hessian3 RTL_V $cname" "tb TESTBENCH_$ext" \
