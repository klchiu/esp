...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        base_addr_1  = 6; // use this for input
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_reshape3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_reshape3");
...
void system_t::dump_memory()
{
    ...
    write_float_matrix_to_file(affine_warp_nxt, 6, 1, "../output/fish_reshape_out.txt", "Add");
...
int system_t::validate()
{
    ...
    for (uint32_t i = 0; i < 6; i++) {
        ...
        if (abs(sd_delta_p_nxt[i] - golden_sd_delta_p_nxt[i]) > 0.001) {
            error_count++;
            ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, sd_delta_p_nxt[i], golden_sd_delta_p_nxt[i]);
        }
        total++;
        // ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, sd_delta_p_nxt[i], golden_sd_delta_p_nxt[i]);
    }
