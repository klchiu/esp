...
#define SLD_ELTMUL 0x072
#define DEV_NAME   "sld,eltmul_vivado"
...
/* <<--regs-->> */
#define ELTMUL_STATE_CONTROL_REG 0x48
#define ELTMUL_MATRIX_DIM_Y_REG   0x44
#define ELTMUL_MATRIX_DIM_X_REG   0x40
...
int main(int argc, char *argv[])
{
    ...
    ndev = 1;
    /*
        ndev = probe(&espdevs, VENDOR_SLD, SLD_ELTMUL, DEV_NAME);
        if (ndev == 0) {
            printf("eltmul not found\n");
            return 0;
        }
    */
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            /* <<--regs-config-->> */
            iowrite32(dev, ELTMUL_MATRIX_DIM_X_REG, matrix_dim_x);
            iowrite32(dev, ELTMUL_MATRIX_DIM_Y_REG, matrix_dim_y);

            state_control = 1; // only load
            iowrite32(dev, ELTMUL_STATE_CONTROL_REG, state_control);
