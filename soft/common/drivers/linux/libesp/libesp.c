...
{
    // return two digit int to indicate which gemm to use, return -1 if using original gemm
...
{
    // lock_a_device2: if find the available accelerator, return the id, else return -1
...
void esp_run(esp_thread_info_t cfg[], unsigned nacc)
{
    ...
    if(strstr(cfg->devname_noid, "gemm") != NULL)       // if we are using gemm
    {
        ...
        if (gemm_size > 0){     // use the fixed size gemm
            sprintf(gemmname_noid, "%s%d", gemm_size);

            dev_id = lock_a_device2(gemmname_noid, cfg->puffinname);
        }
        if (gemm_size == -1){   // use the original gemm
