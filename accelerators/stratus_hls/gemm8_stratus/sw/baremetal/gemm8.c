...
#define SLD_GEMM8 0x051
#define DEV_NAME "sld,gemm8_stratus"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_GEMM8, DEV_NAME);
	if (ndev == 0) {
		printf("gemm8 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, GEMM8_DO_RELU_REG, do_relu);
			iowrite32(dev, GEMM8_TRANSPOSE_REG, transpose);
			iowrite32(dev, GEMM8_NINPUTS_REG, ninputs);
			iowrite32(dev, GEMM8_D3_REG, d3);
			iowrite32(dev, GEMM8_D2_REG, d2);
			iowrite32(dev, GEMM8_D1_REG, d1);
			iowrite32(dev, GEMM8_ST_OFFSET_REG, st_offset);
			iowrite32(dev, GEMM8_LD_OFFSET1_REG, ld_offset1);
			iowrite32(dev, GEMM8_LD_OFFSET2_REG, ld_offset2);
