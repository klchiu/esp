...
#ifndef __WAMI_INV3_FUNCTIONS_HPP__
#define __WAMI_INV3_FUNCTIONS_HPP__

#include "wami_inv3.hpp"

// Compute the 1D x and y gradients of the input matrix / image

#define CLOSE_TO_ZERO(X) (((X) < FPDATA(1e-36)) && ((X) > FPDATA(-1e-36)))

void wami_inv3::swap_row_workspace(uint32_t nRows, uint32_t nCols, uint32_t r1, uint32_t r2)
{
SWAP_ROW_LOOP:
...
void wami_inv3::swap_row_inverse(uint32_t nRows, uint32_t nCols, uint32_t r1, uint32_t r2)
{
SWAP_ROW_LOOP:
    for (uint32_t x = 0; x < nCols; x++) {
        wait(); // STG_STATE_swap_row_0T:
...
void wami_inv3::scale_row_workspace(uint32_t nRows, uint32_t nCols, uint32_t r, FPDATA scale)
{
SCALE_ROW_LOOP:
    for (uint32_t x = 0; x < nCols; x++) {
...
void wami_inv3::scale_row_inverse(uint32_t nRows, uint32_t nCols, uint32_t r, FPDATA scale)
{
SCALE_ROW_LOOP:
    for (uint32_t x = 0; x < nCols; x++) {
        wait(); // STG_STATE_scale_row_0:
...
void wami_inv3::scale_and_add_row_workspace(uint32_t nRows, uint32_t nCols, uint32_t r1, uint32_t r2, FPDATA scale)
{
...
void wami_inv3::scale_and_add_row_inverse(uint32_t nRows, uint32_t nCols, uint32_t r1, uint32_t r2, FPDATA scale)
{
    wait(); // SCALE_AND_ADD_ROW_LOOP:
    for (uint32_t x = 0; x < nCols; x++) {
        wait(); // STG_STATE_scale_and_add_row_0:
...
void wami_inv3::invert_gj(uint32_t nRows, uint32_t nCols)
{
    uint32_t row, col;
    // int      i;
    ...
    for (uint32_t y = 0; y < nRows; y++) {
        for (uint32_t x = 0; x < nCols; x++) {
            ...
            FPDATA_WORD temp01          = A0_hess[y * nCols + x];
...
#endif // __WAMI_INV3_FUNCTIONS_HPP__
