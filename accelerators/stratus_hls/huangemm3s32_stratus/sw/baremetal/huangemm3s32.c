...
#define SLD_HUANGEMM3S32 0x02c
#define DEV_NAME "sld,huangemm3s32_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s32_len*huangemm3s32_vec;
	// 	out_words_adj = huangemm3s32_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s32_len*huangemm3s32_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s32_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s32_n);
	// out_len = out_words_adj * (huangemm3s32_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3S32, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s32 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3S32_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3S32_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3S32_LOADED_COLS_REG, loaded_cols);
