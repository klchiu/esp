...
#define SLD_GEMM16 0x051
#define DEV_NAME "sld,gemm16_stratus"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_GEMM16, DEV_NAME);
	if (ndev == 0) {
		printf("gemm16 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, GEMM16_DO_RELU_REG, do_relu);
			iowrite32(dev, GEMM16_TRANSPOSE_REG, transpose);
			iowrite32(dev, GEMM16_NINPUTS_REG, ninputs);
			iowrite32(dev, GEMM16_D3_REG, d3);
			iowrite32(dev, GEMM16_D2_REG, d2);
			iowrite32(dev, GEMM16_D1_REG, d1);
			iowrite32(dev, GEMM16_ST_OFFSET_REG, st_offset);
			iowrite32(dev, GEMM16_LD_OFFSET1_REG, ld_offset1);
			iowrite32(dev, GEMM16_LD_OFFSET2_REG, ld_offset2);
