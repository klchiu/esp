...
#include "wami_sd_update3_functions.hpp"

// Processes

void wami_sd_update3::load_input()
{
    ...
    for (uint32_t img = 0; img < img_num_img; img++) {
        ...
        uint32_t   length = 6 * img_num_row * img_num_col;
        dma_info_t dma_info_1(index, length, SIZE_WORD);
        this->dma_read_ctrl.put(dma_info_1);    // from steep_descent
        ...
        this->dma_read_ctrl.put(dma_info_2);    // from sub
        ...
        for (unsigned i = 0; i < length; i++) {
            ...
            HLS_BREAK_DEP(A0_subtract);

            FPDATA_WORD pixel_fp;
            pixel_fp = pixel_bv.to_int64();

            A0_subtract[i] = pixel_fp;
            wait();
        }
...
void wami_sd_update3::store_output()
{
    uint32_t img_num_img          = 0;
    ...
    for (uint32_t img = 0; img < img_num_img; img++) {
        ...
        for (unsigned i = 0; i < 6; i++) {
            ...
            HLS_BREAK_DEP(B0_sd_update);
            FPDATA_WORD             pixel_fp;
            sc_dt::sc_bv<WORD_SIZE> pixel_bv;

            wait();
            pixel_fp = B0_sd_update[i];
...
void wami_sd_update3::compute_kernel()
{
    uint32_t img_num_img = 0;
    uint32_t img_num_row = 0;
    uint32_t img_num_col = 0;
    ...
    for (uint16_t n = 0; n < img_num_img; n++) {
        ...
        sd_update(img_num_row, img_num_col, 6);
