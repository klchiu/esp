...
#define SLD_HUANGEMM3S2048 0x02c
#define DEV_NAME "sld,huangemm3s2048_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s2048_len*huangemm3s2048_vec;
	// 	out_words_adj = huangemm3s2048_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s2048_len*huangemm3s2048_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s2048_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s2048_n);
	// out_len = out_words_adj * (huangemm3s2048_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3S2048, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s2048 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3S2048_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3S2048_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3S2048_LOADED_COLS_REG, loaded_cols);
