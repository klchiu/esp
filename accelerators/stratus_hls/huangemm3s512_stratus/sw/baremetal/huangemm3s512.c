...
#define SLD_HUANGEMM3S512 0x02c
#define DEV_NAME "sld,huangemm3s512_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s512_len*huangemm3s512_vec;
	// 	out_words_adj = huangemm3s512_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s512_len*huangemm3s512_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s512_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s512_n);
	// out_len = out_words_adj * (huangemm3s512_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3S512, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s512 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3S512_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3S512_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3S512_LOADED_COLS_REG, loaded_cols);
