...
#define DEVNAME "/dev/nightDwt_stratus.0"
#define NAME    "nightDwt_stratus"
...
static const char usage_str[] = "usage: ./nightDwt_stratus.exe coherence infile [nimages] [ncols] [nrows] [-v]\n"
...
struct nightDwt_test {
    struct test_info                  info;
    struct nightDwt_stratus_access desc;
    char *                            infile;
    bool                              infile_is_raw;
    unsigned                          nimages;
    unsigned                          rows;
...
static inline struct nightDwt_test *to_nightDwt(struct test_info *info)
{
    return container_of(info, struct nightDwt_test, info);
}
...
static void init_buf(struct nightDwt_test *t)
{
...
static inline size_t nightDwt_size(struct nightDwt_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(int);
}

static inline size_t nightDwt_size_opt(struct nightDwt_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(short);
}

static void nightDwt_alloc_buf(struct test_info *info)
{
    struct nightDwt_test *t = to_nightDwt(info);

    t->hbuf = malloc0_check(nightDwt_size_opt(t));
    if (!strcmp(info->cmd, "test")) {
        t->sbuf_in  = malloc0_check(nightDwt_size(t));
        t->sbuf_out = malloc0_check(nightDwt_size(t));
    }
}

static void nightDwt_alloc_contig(struct test_info *info)
{
    struct nightDwt_test *t = to_nightDwt(info);

    printf("HW buf size: %zu B\n", nightDwt_size(t));
    if (contig_alloc(nightDwt_size(t), &info->contig) == NULL)
        die_errno(__func__);
}

static void nightDwt_init_bufs(struct test_info *info)
{
    struct nightDwt_test *t = to_nightDwt(info);

    init_buf(t);
    contig_copy_to(info->contig, 0, t->hbuf, nightDwt_size_opt(t));
}

static void nightDwt_set_access(struct test_info *info)
{
    struct nightDwt_test *t = to_nightDwt(info);
...
static void nightDwt_comp(struct test_info *info)
{
    struct nightDwt_test *t      = to_nightDwt(info);
    int                      i      = 0;
...
static bool nightDwt_diff_ok(struct test_info *info)
{
    struct nightDwt_test *t         = to_nightDwt(info);
    int                      total_err = 0;
    int                      i;

    contig_copy_from(t->hbuf, info->contig, 0, nightDwt_size_opt(t));
...
static struct nightDwt_test nightDwt_test = {
    .info =
        {
            .name         = NAME,
            .devname      = DEVNAME,
            .alloc_buf    = nightDwt_alloc_buf,
            .alloc_contig = nightDwt_alloc_contig,
            .init_bufs    = nightDwt_init_bufs,
            .set_access   = nightDwt_set_access,
            .comp         = nightDwt_comp,
            .diff_ok      = nightDwt_diff_ok,
            .esp          = &nightDwt_test.desc.esp,
            .cm           = NIGHTDWT_STRATUS_IOC_ACCESS,
        },
};
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3 || argc == 5 || argc > 7) {
        ...
        nightDwt_test.infile = argv[2];
        printf("\tinfile: %s\n", nightDwt_test.infile);

        if (argc == 6 || argc == 7) {
            nightDwt_test.nimages = strtol(argv[3], NULL, 10);
            nightDwt_test.cols    = strtol(argv[4], NULL, 10);
            nightDwt_test.rows    = strtol(argv[5], NULL, 10);
            printf("\tnimages: %u\n", nightDwt_test.nimages);
            printf("\tncols: %u\n", nightDwt_test.cols);
            printf("\tnrows: %u\n", nightDwt_test.rows);
        } else {
            nightDwt_test.nimages = DEFAULT_NIMAGES;
            nightDwt_test.rows    = DEFAULT_NROWS;
            nightDwt_test.cols    = DEFAULT_NCOLS;
        }
        ...
        if (argc == 4 || argc == 7) {
            if ((strcmp(argv[3], "-v") && argc == 4) || (strcmp(argv[6], "-v") && argc == 7)) {
                ...
                nightDwt_test.verbose = true;
                printf("\tverbose enabled\n");
            }
        }
        nightDwt_test.nbytespp      = DEFAULT_NBYTESPP;
        nightDwt_test.swapbytes     = DEFAULT_SWAPBYTES;
        nightDwt_test.do_dwt        = DEFAULT_DO_DWT;
        nightDwt_test.infile_is_raw = DEFAULT_INFILE_IS_RAW;
        nightDwt_test.nbpp_in       = DEFAULT_NBPP_IN;
        nightDwt_test.nbpp_out      = DEFAULT_NBPP_OUT;
        printf("\n");
    }

    return test_main(&nightDwt_test.info, argv[1], "test");
}
