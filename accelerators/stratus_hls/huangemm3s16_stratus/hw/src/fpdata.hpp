...
typedef sc_dt::sc_int<WORD_SIZE> DATA_WORD; 
typedef sc_dt::sc_bv<WORD_SIZE> BV_WORD;



// int2bv
template<typename T, size_t N>
T int2bv(sc_dt::sc_int<N> data_in)
{
    T data_out;

    {
        ...
        {
            HLS_UNROLL_LOOP(ON, "int2bv-loop");
...
template<typename T, size_t N>
T bv2int(sc_dt::sc_bv<N> data_in)
{
    T data_out;

    {
        ...
        for (unsigned i = 0; i < N; i++)
        {
            HLS_UNROLL_LOOP(ON, "bv2int-loop");
