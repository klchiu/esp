...
#include "wami_reshape3_functions.hpp"

// Processes

void wami_reshape3::load_input()
{
...
void wami_reshape3::store_output()
{
    uint32_t img_num_img          = 0;
    ...
    for (uint32_t img = 0; img < img_num_img; img++) {
        ...
        for (unsigned i = 0; i < length; i++) {
            ...
            HLS_BREAK_DEP(plm_B0_sd_delta_p);
            FPDATA_WORD             pixel_fp;
            sc_dt::sc_bv<WORD_SIZE> pixel_bv;

            wait();
            pixel_fp = plm_B0_sd_delta_p[i];
...
void wami_reshape3::compute_kernel()
{
    uint32_t img_num_img = 0;
    // Reset
    {
