...
{
    base_addr = 0; // [humu]: use this for now, for output 1
    base_addr_1 =
        6 * (INPUT_IMG_NUM_ROWS - 2 * PAD) * (INPUT_IMG_NUM_COLS - 2 * PAD); // [humu]: use this for now, for output 2
    base_addr_2 = base_addr_1 + 36;                                          // [humu]: use this for now, for input 1
    base_addr_3 = base_addr_2 + (INPUT_IMG_NUM_ROWS - 2 * PAD) *
                                    (INPUT_IMG_NUM_COLS - 2 * PAD); // [humu]: use this for now, for input 2
    base_addr_4 = 1;    // [humu]: use this for num_output_copy for now
    ...
    // Config
    {
        conf_info_t config(base_addr, nTestImages, nCols - 2 * PAD, nRows - 2 * PAD, padding_in, ADD_KERN_ID,
                           base_addr_1, base_addr_2, base_addr_3, base_addr_4);
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_steep_descent_hessian");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_steep_descent_hessian");
...
void system_t::load_memory()
{
    ...
    ESP_REPORT_INFO("[load_memory 1]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < (nRows - 2 * PAD) * (nCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;

        // convert float to fx
        data_fp = FPDATA(golden_warp_dx[index]);
        // convert fx to bv
    ...
    // Load input 2
    mem_base_addr = base_addr_3;
    ESP_REPORT_INFO("[load_memory 2]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < (nRows - 2 * PAD) * (nCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;

        // convert float to fx
        data_fp = FPDATA(golden_warp_dy[index]);
        // convert fx to bv
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < 6 * (nRows - 2 * PAD) * (nCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
        ...
        // conver fp to float
        I_steepest[index] = (float)data_fp;
    }
    write_float_matrix_to_file(I_steepest, 6 * (nRows - 2 * PAD) * (nCols - 2 * PAD), 1,
                               "../output/fish_steep_descent_out.txt", "Steep Descent");
    ...
    for (uint32_t index = 0; index < 36; index++) {
        ...
        data_fp = bv2fp<FPDATA, FPDATA_WL>(data_bv);
        // conver fp to float
        hess[index] = (float)data_fp;
    }
    write_float_matrix_to_file(hess, 36, 1, "../output/fish_hessian_out.txt", "Hessian");
...
int system_t::validate()
{
    ...
    for (int i = 0; i < 6 * (nRows - 2 * PAD) * (nCols - 2 * PAD); i++) {
        if (abs(I_steepest[i] - golden_I_steepest[i]) / golden_I_steepest[i] > 0.01) {
            error_count++;
            ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, I_steepest[i], golden_I_steepest[i]);
        }
        total++;
        // ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, I_steepest[i], golden_I_steepest[i]);
    }
    ...
    ESP_REPORT_INFO("====================================================");
    if (error_count > 0) {
        ESP_REPORT_INFO("Errors (steepest): %d out of %d pixel(s) not match.", error_count, total);
    } else {
        ESP_REPORT_INFO("Correct!! (steepest)");
    }
    ESP_REPORT_INFO("====================================================");
    ...
    for (uint32_t i = 0; i < 36; i++) {
        float error_rate = abs((hess[i] - golden_hess[i]) / golden_hess[i]);
        if (error_rate > 0.01) {
            error_count++;
            ESP_REPORT_INFO("%d: out: %f\tgold: %f\tdiff: %f", i, hess[i], golden_hess[i], error_rate);
        }
        total++;
        // ESP_REPORT_INFO("%d: out: %f\tgold: %f\tdiff: %f", i, hess[i], golden_hess[i], error_rate);
    }
    ...
    ESP_REPORT_INFO("====================================================");
    if (error_count > 0) {
        ESP_REPORT_INFO("Errors (hessian): %d out of %d pixel(s) not match.", error_count, total);
    } else {
        ESP_REPORT_INFO("Correct!! (hessian)");
    }
    ESP_REPORT_INFO("====================================================");
