...
#ifdef CADENCE
    #include "wami_reshape_add_wrap.h"
#endif
...
class system_t : public esp_system<DMA_WIDTH, 5000000>
{
  public:
    // ACC instance
#ifdef CADENCE
    wami_reshape_add_wrapper *acc;
#else
    wami_reshape_add *acc;
#endif
        ...
        : esp_system<DMA_WIDTH, 5000000>(name)
    {
        // ACC
#ifdef CADENCE
        acc = new wami_reshape_add_wrapper("wami_reshape_add_wrapper");
#else
        acc = new wami_reshape_add("wami_reshape_add_wrapper");
#endif
        // Binding ACC
        acc->clk(clk);
        acc->rst(acc_rst);
