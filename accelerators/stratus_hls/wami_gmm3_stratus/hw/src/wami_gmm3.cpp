// Copyright (c) 2011-2021 Columbia University, System Level Design Group
// SPDX-License-Identifier: Apache-2.0

#include "wami_gmm3.hpp"
#include "wami_gmm3_directives.hpp"


// Functions

#include "wami_gmm3_functions.hpp"

// Processes

void wami_gmm3::load_input()
{
    uint32_t img_num_img          = 0;
    uint32_t img_num_col          = 0;
    uint32_t img_num_row          = 0;
    uint32_t mem_src_dst_offset_1 = 0;
    uint32_t mem_src_dst_offset_2 = 0;
    uint32_t mem_src_dst_offset_3 = 0;
    uint32_t mem_src_dst_offset_4 = 0;
    uint32_t is_p2p               = 0;
    uint32_t index, length;

    // Reset
    {
        HLS_PROTO("load-reset");

        this->reset_load_input();
        accel_ready.ack.reset_ack();

        // User-defined reset code

        wait();
    }

    // Config
    {
        HLS_PROTO("load-config");

        cfg.wait_for_config(); // config process
        conf_info_t config = this->conf_info.read();

        // User-defined config code
        img_num_img          = config.num_img;
        img_num_col          = config.num_col;
        img_num_row          = config.num_row;
        mem_src_dst_offset_1 = config.src_dst_offset_1;
        mem_src_dst_offset_2 = config.src_dst_offset_2;
        mem_src_dst_offset_3 = config.src_dst_offset_3;
        mem_src_dst_offset_4 = config.src_dst_offset_4;
        is_p2p               = config.is_p2p;
    }

    // Load
    for (uint32_t img = 0; img < img_num_img; img++) {

        HLS_PROTO("load-dma");

        this->load_store_handshake();

        // == BURST (read first input, gmm_img) ==
        index  = mem_src_dst_offset_1;
        length = img_num_col * img_num_row; // 16384
        dma_info_t dma_info_1(index, length, SIZE_WORD);
        this->dma_read_ctrl.put(dma_info_1);

        for (unsigned i = 0; i < length; i++) {
            sc_dt::sc_bv<DMA_WIDTH> pixel_bv = this->dma_read_chnl.get();
            wait();

            HLS_BREAK_DEP(plm_A0_gmm_img);

            uint16_t tmp = pixel_bv.to_uint();

            plm_A0_gmm_img[i] = tmp;
            wait();
        }

        if (is_p2p == 0) {
            // == BURST (read input, mu) ==
            index  = mem_src_dst_offset_2;
            length = 5 * img_num_col * img_num_row; // 5*16384
            dma_info_t dma_info_2(index, length, SIZE_WORD);
            this->dma_read_ctrl.put(dma_info_2);

            for (unsigned i = 0; i < length; i++) {
                sc_dt::sc_bv<DMA_WIDTH> pixel_bv = this->dma_read_chnl.get();
                wait();

                HLS_BREAK_DEP(plm_A0_mu);

                FPDATA_WORD pixel_fp;
                pixel_fp = pixel_bv.range(63, 0).to_int64();

                plm_A0_mu[i] = pixel_fp;
                wait();
            }

            // == BURST (read input, sigma) ==
            index  = mem_src_dst_offset_3;
            length = 5 * img_num_col * img_num_row; // 5*16384
            dma_info_t dma_info_3(index, length, SIZE_WORD);
            this->dma_read_ctrl.put(dma_info_3);

            for (unsigned i = 0; i < length; i++) {
                sc_dt::sc_bv<DMA_WIDTH> pixel_bv = this->dma_read_chnl.get();
                wait();

                HLS_BREAK_DEP(plm_A0_sigma);

                FPDATA_WORD pixel_fp;
                pixel_fp = pixel_bv.range(63, 0).to_int64();

                plm_A0_sigma[i] = pixel_fp;
                wait();
            }

            // == BURST (read input, weight) ==
            index  = mem_src_dst_offset_4;
            length = 5 * img_num_col * img_num_row; // 5*16384
            dma_info_t dma_info_4(index, length, SIZE_WORD);
            this->dma_read_ctrl.put(dma_info_4);

            for (unsigned i = 0; i < length; i++) {
                sc_dt::sc_bv<DMA_WIDTH> pixel_bv = this->dma_read_chnl.get();
                wait();

                HLS_BREAK_DEP(plm_A0_weight);

                FPDATA_WORD pixel_fp;
                pixel_fp = pixel_bv.range(63, 0).to_int64();

                plm_A0_weight[i] = pixel_fp;
                wait();
            }
        }
        if (is_p2p == 1) {
            // when p2p is enable, don't read mu, sigma, weight from other acc for now

            length = 5 * img_num_col * img_num_row; // 5*16384
            for (unsigned i = 0; i < length; i++) {
                HLS_BREAK_DEP(plm_A0_mu);

                plm_A0_mu[i] = 0;
                wait();
            }

            length = 5 * img_num_col * img_num_row; // 5*16384
            for (unsigned i = 0; i < length; i++) {
                HLS_BREAK_DEP(plm_A0_sigma);

                plm_A0_sigma[i] = 0;
                wait();
            }

            length = 5 * img_num_col * img_num_row; // 5*16384
            for (unsigned i = 0; i < length; i++) {
                HLS_BREAK_DEP(plm_A0_weight);

                plm_A0_weight[i] = 0;
                wait();
            }
        }

        this->load_compute_handshake();
    }

    // Conclude
    {
        HLS_PROTO("load-done");
        this->process_done();
    }
}

void wami_gmm3::store_output()
{
    uint32_t img_num_img          = 0;
    uint32_t img_num_col          = 0;
    uint32_t img_num_row          = 0;
    uint32_t mem_src_dst_offset_0 = 0;
    uint32_t index, length;

    // Reset
    {
        HLS_PROTO("store-reset");

        this->reset_store_output();
        accel_ready.req.reset_req();

        // User-defined reset code
        wait();
    }

    // Config
    {
        HLS_PROTO("store-config");

        cfg.wait_for_config(); // config process
        conf_info_t config = this->conf_info.read();

        // User-defined config code
        img_num_img          = config.num_img;
        img_num_col          = config.num_col;
        img_num_row          = config.num_row;
        mem_src_dst_offset_0 = config.src_dst_offset_0;
    }

    for (uint32_t img = 0; img < img_num_img; img++) {
        HLS_PROTO("store-dma");

        this->store_load_handshake();
        this->store_compute_handshake();

        // Configure DMA write
        index  = mem_src_dst_offset_0;
        length = img_num_col * img_num_row;
        dma_info_t dma_info(index, length, SIZE_WORD);
        this->dma_write_ctrl.put(dma_info);

        for (unsigned i = 0; i < length; i++) {

            HLS_BREAK_DEP(plm_B0_foreground);
            sc_dt::sc_bv<WORD_SIZE> pixel_bv;

            wait();
            uint64_t temp = plm_B0_foreground[i];

            pixel_bv.range(63, 0) = temp;

            wait();
            this->dma_write_chnl.put(pixel_bv);
        }
    }

    // Conclude
    {
        HLS_PROTO("store-done");
        this->accelerator_done();
        this->process_done();
    }
}

void wami_gmm3::compute_kernel()
{
    uint32_t img_num_img = 0;
    uint32_t img_num_col = 0;
    uint32_t img_num_row = 0;
    uint32_t nModels     = 0;
    // Reset
    {
        HLS_PROTO("compute-reset");

        this->reset_compute_kernel();

        // User-defined reset code
        wait();
    }

    {
        HLS_PROTO("compute-config");

        cfg.wait_for_config(); // config process
        conf_info_t config = this->conf_info.read();

        // User-defined config code
        img_num_img = config.num_img;
        img_num_col = config.num_col;
        img_num_row = config.num_row;
        nModels     = config.batch; // [humu]: TODO: change batch to nModels
    }

    // Compute
    for (uint16_t n = 0; n < img_num_img; n++) {
        HLS_UNROLL_LOOP(OFF);

        this->compute_load_handshake();

        // Computing phase implementation
        // kernel_reshape(6, 1, 2, 3);
        kernel_gmm(img_num_row, img_num_col, nModels);

        this->compute_store_handshake();
    }

    // Conclude
    {
        HLS_PROTO("compute-done");
        this->process_done();
    }
}
