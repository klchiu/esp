...
#define SLD_HUANGEMM3s10000 0x02c
#define DEV_NAME "sld,huangemm3s10000_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s10000_len*huangemm3s10000_vec;
	// 	out_words_adj = huangemm3s10000_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s10000_len*huangemm3s10000_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s10000_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s10000_n);
	// out_len = out_words_adj * (huangemm3s10000_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3s10000, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s10000 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3s10000_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3s10000_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3s10000_LOADED_COLS_REG, loaded_cols);
