...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id     = DEBAYER_KERN_ID;
        batch       = 0;
        base_addr_0 = 0;                 // use this for output
        base_addr_1 = num_col * num_row; // use this for input
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_debayer3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_debayer3");
...
void system_t::load_memory()
{
    ...
    uint32_t mem_base_addr = base_addr_1;
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);

    for (uint32_t index = 0; index < srcRows * srcCols; index++) {
        uint64_t         image64 = images[index];
        sc_dt::sc_bv<64> data_bv(image64);
...
void system_t::dump_memory()
{
    ...
    uint32_t mem_base_addr = base_addr_0;
    ESP_REPORT_INFO("[dump_memory]: mem_base_addr: %d", mem_base_addr);

    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        data_bv               = mem[mem_base_addr + index];
        uint64_t    pixel     = data_bv.to_uint64();
        rgb_pixel_t rgb_pixel = {0, 0, 0};
        rgb_pixel.r           = (pixel >> 32) & 0xFFFF;
        rgb_pixel.g           = (pixel >> 16) & 0xFFFF;
        rgb_pixel.b           = (pixel >> 0) & 0xFFFF;
        rgb_imgs[index]      = rgb_pixel;
    }

    // write_rgb_matrix_to_file(rgb_imgs, WAMI_GRAYSCALE_IMG_NUM_ROWS, WAMI_GRAYSCALE_IMG_NUM_COLS,
    // "../output/fish_debayer_out.txt", "Fish RGB");
...
int system_t::validate()
{
    ...
    // Debayer - validation
    for (uint32_t row = 0; row < srcRows - 2 * PAD; row++) {
        for (uint32_t col = 0; col < srcCols - 2 * PAD; col++) {
            uint32_t i = row * (srcCols - 2 * PAD) + col;

            if (golden_rgb_imgs[i].r != rgb_imgs[i].r || golden_rgb_imgs[i].g != rgb_imgs[i].g ||
                golden_rgb_imgs[i].b != rgb_imgs[i].b) {
                ESP_REPORT_INFO("%d: out: %d\tgold: %d", i, rgb_imgs[i].r, golden_rgb_imgs[i].r);
                ESP_REPORT_INFO("%d: out: %d\tgold: %d", i, rgb_imgs[i].g, golden_rgb_imgs[i].g);
                ESP_REPORT_INFO("%d: out: %d\tgold: %d", i, rgb_imgs[i].b, golden_rgb_imgs[i].b);
                error_count++;
            }
            total++;
            // ESP_REPORT_INFO("%d: out: %d\tgold: %d", i, rgb_imgs[i].r, golden_rgb_imgs[i].r);
