...
/**
 * Baremetal device driver for NIGHTNF
 *
...
#define SLD_NIGHTNF 0x13
#define DEV_NAME        "sld,nightNF_stratus"
...
#define NIGHTNF_BUF_SIZE (ROWS * COLS * 2 * sizeof(pixel))

/* Size of the contiguous chunks for scatter/gather */
#define CHUNK_SHIFT 10
#define CHUNK_SIZE  BIT(CHUNK_SHIFT)
#define NCHUNK                                                                      \
    ((NIGHTNF_BUF_SIZE % CHUNK_SIZE == 0) ? (NIGHTNF_BUF_SIZE / CHUNK_SIZE) \
                                              : (NIGHTNF_BUF_SIZE / CHUNK_SIZE) + 1)

// User defined registers
#define NIGHTNF_NIMAGES_REG 0x40
#define NIGHTNF_NROWS_REG   0x44
#define NIGHTNF_NCOLS_REG   0x48
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_NIGHTNF, DEV_NAME);
    if (ndev <= 0) {
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            mem = aligned_malloc(NIGHTNF_BUF_SIZE);
    ...
    #include "data_480x640.h"
            ...
            iowrite32(dev, NIGHTNF_NIMAGES_REG, 1);
            iowrite32(dev, NIGHTNF_NROWS_REG, ROWS);
            iowrite32(dev, NIGHTNF_NCOLS_REG, COLS);
#if (ROWS == 32 && COLS == 32)
            iowrite32(dev, NIGHTNF_DO_DWT_REG, 0);
#else
            iowrite32(dev, NIGHTNF_DO_DWT_REG, 1);
#endif
