...
#define SLD_CONV2DU8 0x052
#define DEV_NAME "sld,conv2dU8_stratus"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_CONV2DU8, DEV_NAME);
	if (ndev == 0) {
		printf("conv2dU8 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, CONV2DU8_N_CHANNELS_REG, n_channels);
			iowrite32(dev, CONV2DU8_FEATURE_MAP_HEIGHT_REG, feature_map_height);
			iowrite32(dev, CONV2DU8_FEATURE_MAP_WIDTH_REG, feature_map_width);
			iowrite32(dev, CONV2DU8_N_FILTERS_REG, n_filters);
			iowrite32(dev, CONV2DU8_FILTER_DIM_REG, filter_height);
			iowrite32(dev, CONV2DU8_IS_PADDED_REG, is_padded);
			iowrite32(dev, CONV2DU8_STRIDE_REG, stride_w);
			iowrite32(dev, CONV2DU8_DO_RELU_REG, do_relu);
			iowrite32(dev, CONV2DU8_POOL_TYPE_REG, pool_type);
			iowrite32(dev, CONV2DU8_BATCH_SIZE_REG, batch_size);
