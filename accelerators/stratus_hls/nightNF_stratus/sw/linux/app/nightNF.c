...
#define DEVNAME "/dev/nightNF_stratus.0"
#define NAME    "nightNF_stratus"
...
static const char usage_str[] = "usage: ./nightNF_stratus.exe coherence infile [nimages] [ncols] [nrows] [-v]\n"
...
struct nightNF_test {
    ...
    struct nightNF_stratus_access desc;
    char *                            infile;
    bool                              infile_is_raw;
    unsigned                          nimages;
    unsigned                          rows;
...
static inline struct nightNF_test *to_nightNF(struct test_info *info)
{
    return container_of(info, struct nightNF_test, info);
}
...
static void init_buf(struct nightNF_test *t)
{
...
static inline size_t nightNF_size(struct nightNF_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(int);
}

static inline size_t nightNF_size_opt(struct nightNF_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(short);
}

static void nightNF_alloc_buf(struct test_info *info)
{
    struct nightNF_test *t = to_nightNF(info);

    t->hbuf = malloc0_check(nightNF_size_opt(t));
    if (!strcmp(info->cmd, "test")) {
        t->sbuf_in  = malloc0_check(nightNF_size(t));
        t->sbuf_out = malloc0_check(nightNF_size(t));
    }
}

static void nightNF_alloc_contig(struct test_info *info)
{
    struct nightNF_test *t = to_nightNF(info);

    printf("HW buf size: %zu B\n", nightNF_size(t));
    if (contig_alloc(nightNF_size(t), &info->contig) == NULL)
        die_errno(__func__);
}

static void nightNF_init_bufs(struct test_info *info)
{
    struct nightNF_test *t = to_nightNF(info);

    init_buf(t);
    contig_copy_to(info->contig, 0, t->hbuf, nightNF_size_opt(t));
}

static void nightNF_set_access(struct test_info *info)
{
    struct nightNF_test *t = to_nightNF(info);
...
static void nightNF_comp(struct test_info *info)
{
    struct nightNF_test *t      = to_nightNF(info);
    int                      i      = 0;
...
static bool nightNF_diff_ok(struct test_info *info)
{
    struct nightNF_test *t         = to_nightNF(info);
    int                      total_err = 0;
    int                      i;

    contig_copy_from(t->hbuf, info->contig, 0, nightNF_size_opt(t));
...
static struct nightNF_test nightNF_test = {
    .info =
        {
            .name         = NAME,
            .devname      = DEVNAME,
            .alloc_buf    = nightNF_alloc_buf,
            .alloc_contig = nightNF_alloc_contig,
            .init_bufs    = nightNF_init_bufs,
            .set_access   = nightNF_set_access,
            .comp         = nightNF_comp,
            .diff_ok      = nightNF_diff_ok,
            .esp          = &nightNF_test.desc.esp,
            .cm           = NIGHTNF_STRATUS_IOC_ACCESS,
        },
};
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3 || argc == 5 || argc > 7) {
        ...
        nightNF_test.infile = argv[2];
        printf("\tinfile: %s\n", nightNF_test.infile);

        if (argc == 6 || argc == 7) {
            nightNF_test.nimages = strtol(argv[3], NULL, 10);
            nightNF_test.cols    = strtol(argv[4], NULL, 10);
            nightNF_test.rows    = strtol(argv[5], NULL, 10);
            printf("\tnimages: %u\n", nightNF_test.nimages);
            printf("\tncols: %u\n", nightNF_test.cols);
            printf("\tnrows: %u\n", nightNF_test.rows);
        } else {
            nightNF_test.nimages = DEFAULT_NIMAGES;
            nightNF_test.rows    = DEFAULT_NROWS;
            nightNF_test.cols    = DEFAULT_NCOLS;
        }
        ...
        if (argc == 4 || argc == 7) {
            if ((strcmp(argv[3], "-v") && argc == 4) || (strcmp(argv[6], "-v") && argc == 7)) {
                ...
                nightNF_test.verbose = true;
                printf("\tverbose enabled\n");
            }
        }
        nightNF_test.nbytespp      = DEFAULT_NBYTESPP;
        nightNF_test.swapbytes     = DEFAULT_SWAPBYTES;
        nightNF_test.do_dwt        = DEFAULT_DO_DWT;
        nightNF_test.infile_is_raw = DEFAULT_INFILE_IS_RAW;
        nightNF_test.nbpp_in       = DEFAULT_NBPP_IN;
        nightNF_test.nbpp_out      = DEFAULT_NBPP_OUT;
        printf("\n");
    }

    return test_main(&nightNF_test.info, argv[1], "test");
}
