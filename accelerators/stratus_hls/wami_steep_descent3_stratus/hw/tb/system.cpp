...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id      = STEEPEST_DESCENT_KERN_ID;
        batch        = 0;
        base_addr_0  = 0;                               // use this for output
        base_addr_1  = 6 * num_col * num_row;           // use this for input 1
        base_addr_2  = base_addr_1 + num_col * num_row; // use this for input 2
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_steep_descent3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_steep_descent3");
...
void system_t::load_memory()
{
    ...
    // Load input 1
    mem_base_addr = base_addr_1; // 6*128*128
    ...
    // Load input 2
    mem_base_addr = base_addr_2; // 6*128*128+128*128
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < 6 * (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
    ...
    write_float_matrix_to_file(I_steepest, 6 * (srcRows - 2 * PAD) * (srcCols - 2 * PAD), 1,
...
int system_t::validate()
{
    ...
    for (int i = 0; i < 6 * (srcRows - 2 * PAD) * (srcCols - 2 * PAD); i++) {
