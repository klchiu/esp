...
void system_t::malloc_arrays()
{
    ...
    golden_warp_iwxp      = (flt_pixel_t *)malloc(sizeof(flt_pixel_t) * WARP_TOTAL_OUTPUT_NUM_PXL);
        golden_gmm_img = (uint16_t *)malloc(sizeof(uint16_t) * CHANGE_DETECTION_TOTAL_OUTPUT_NUM_PXL);
    ...
    foreground      = (uint64_t *)malloc(sizeof(uint64_t) * img_num_pxls);
...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id      = RESHAPE_KERN_ID;
        batch        = nModels;                             // use this for nModels = 5
        base_addr_0  = 0;                                   // use this for output
        base_addr_1  = num_col * num_row;                   // use this for input 1 (golden_gmm_img or golden_warp_iwxp)
        base_addr_2  = base_addr_1 + num_col * num_row;     // use this for input 2 (mu)
        base_addr_3  = base_addr_2 + 5 * num_col * num_row; // use this for input 3 (mu)
        base_addr_4  = base_addr_3 + 5 * num_col * num_row; // use this for input 4 (mu)
        is_p2p       = 0;
        p2p_config_0 = 0;
        p2p_config_1 = 0;
        ...
        ESP_REPORT_INFO("MEM_SIZE = %d", MEM_SIZE);
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_gmm3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_gmm3");
...
void system_t::load_memory()
{
    ...
    mem_base_addr = base_addr_1; // for golden_gmm_img (golden_warp_iwxp)
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < num_col * num_row; index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;

        uint64_t gmm_img_64 = golden_gmm_img[index];
        data_bv.range(63, 0) = gmm_img_64;

        mem[mem_base_addr + index].range(63, 0) = data_bv;
    }

    mem_base_addr = base_addr_2; // for mu (from bin)
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < 5 * num_col * num_row; index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;

        // convert float to fx
        data_fp = FPDATA(mu[index]);
        // convert fx to bv
        data_bv.range(63, 0) = fp2bv<FPDATA, FPDATA_WL>(data_fp);
    ...
    mem_base_addr = base_addr_3; // for sigma (from bin)
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < 5 * num_col * num_row; index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
    ...
    mem_base_addr = base_addr_4; // for weight (from bin)
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < 5 * num_col * num_row; index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
...
void system_t::dump_memory()
{
    ...
    //     // conver fp to float
    //     sd_delta_p_nxt[index] = (float)data_fp;
    // }
    ...
    for (uint32_t index = 0; index < num_col * num_row; index++) {
        ...
        // data_fp = bv2fp<FPDATA, FPDATA_WL>(data_bv);
        // conver fp to float
        uint64_t temp     = data_bv.to_uint64();
        // if (index < 20){
        //     fprintf(stderr, "[dump memory]: %d --> %d\n", index, temp);
        // }
        foreground[index] = temp;//  (uint8_t)temp;
    }
...
int system_t::validate()
{
    ...
    int total       = 0;
     ...
     for (uint32_t i = 0; i < num_col * num_row; i++) {
    // for (uint32_t i = 0; i < 20; i++) {

        if (abs(foreground[i] - golden_foreground[i]) > 0.01) {
            error_count++;
            ESP_REPORT_ERROR("%d: out: %d\tgold: %d", i, foreground[i], golden_foreground[i]);
        }
        total++;
        //ESP_REPORT_INFO("%d: out: %d\tgold: %d", i, foreground[i], golden_foreground[i]);
    }
    ...
    // int      errors = 0;
    // ESP_REPORT_INFO("CHANGE_DETECTION_TOTAL_OUTPUT_NUM_PXL: %d", CHANGE_DETECTION_TOTAL_OUTPUT_NUM_PXL);
    // for (int i = 0; i < CHANGE_DETECTION_TOTAL_OUTPUT_NUM_PXL; i++) {
    //     errors += golden_foreground[i];
    //     if (golden_foreground[i] != 0) {
    //         ESP_REPORT_INFO("golden_foreground[%d] = %d", i, golden_foreground[i]);
    //     }
    // }
    // ESP_REPORT_INFO("errors: %d", errors);
