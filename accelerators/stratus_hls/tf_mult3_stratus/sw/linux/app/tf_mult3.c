...
static void init_parameters(int len)
{
    ...
    struct tf_mult3_stratus_access *tmp;
    tmp = (struct tf_mult3_stratus_access *)cfg_tf_mult3[0].esp_desc;
    ...
    // tf_mult3_cfg_000[0].tf_src_dst_offset_0 = base_addr_0;
	// tf_mult3_cfg_000[0].tf_src_dst_offset_1 = base_addr_1;
	// tf_mult3_cfg_000[0].tf_src_dst_offset_2 = base_addr_2;

    printf("  %s parameters\n", cfg_tf_mult3[0].devname);
...
int run_test(int test_len, unsigned long long *hw_ns, unsigned long long *sw_ns)
{
    ...
    gettime(&t_hw_start);
	esp_run_no_print(cfg_tf_mult3, 1);
	// esp_run(cfg_tf_mult3, 1);
    gettime(&t_hw_end);
...
{
    FILE* log_0320 = fopen("log_0320_tf_mult3.txt", "w");
