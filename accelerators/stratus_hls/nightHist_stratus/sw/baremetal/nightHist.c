...
/**
 * Baremetal device driver for NIGHTHIST
 *
...
#define SLD_NIGHTHIST 0x13
#define DEV_NAME        "sld,nightHist_stratus"
...
#define NIGHTHIST_BUF_SIZE (ROWS * COLS * 2 * sizeof(pixel))

/* Size of the contiguous chunks for scatter/gather */
#define CHUNK_SHIFT 10
#define CHUNK_SIZE  BIT(CHUNK_SHIFT)
#define NCHUNK                                                                      \
    ((NIGHTHIST_BUF_SIZE % CHUNK_SIZE == 0) ? (NIGHTHIST_BUF_SIZE / CHUNK_SIZE) \
                                              : (NIGHTHIST_BUF_SIZE / CHUNK_SIZE) + 1)

// User defined registers
#define NIGHTHIST_NIMAGES_REG 0x40
#define NIGHTHIST_NROWS_REG   0x44
#define NIGHTHIST_NCOLS_REG   0x48
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_NIGHTHIST, DEV_NAME);
    if (ndev <= 0) {
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            mem = aligned_malloc(NIGHTHIST_BUF_SIZE);
    ...
    #include "data_480x640.h"
            ...
            iowrite32(dev, NIGHTHIST_NIMAGES_REG, 1);
            iowrite32(dev, NIGHTHIST_NROWS_REG, ROWS);
            iowrite32(dev, NIGHTHIST_NCOLS_REG, COLS);
#if (ROWS == 32 && COLS == 32)
            iowrite32(dev, NIGHTHIST_DO_DWT_REG, 0);
#else
            iowrite32(dev, NIGHTHIST_DO_DWT_REG, 1);
#endif
