...
void system_t::config_proc()
{
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_grayscale3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_grayscale3");
...
void system_t::load_memory()
{
    ...
    uint32_t mem_base_addr = base_addr_1; // 128*128
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);

    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {

        uint16_t pixel_r = golden_rgb_imgs[index].r;
        uint16_t pixel_g = golden_rgb_imgs[index].g;
        uint16_t pixel_b = golden_rgb_imgs[index].b;

        // each dma [63, 48] should be empty
        mem[mem_base_addr + index].range(47, 32) = sc_bv<16>(pixel_r);
        mem[mem_base_addr + index].range(31, 16) = sc_bv<16>(pixel_g);
        mem[mem_base_addr + index].range(15, 0)  = sc_bv<16>(pixel_b);
    }
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        data_bv        = mem[mem_base_addr + index].range(63, 0);
        FPDATA data_fp = bv2fp<FPDATA, WORD_SIZE>(data_bv);

        gs_imgs[mem_base_addr + index] = (float)data_fp;
    }
    write_float_matrix_to_file(gs_imgs, srcRows - 2 * PAD, srcCols - 2 * PAD, "../output/fish_grayscale_out.txt", "Grayscale");
...
int system_t::validate()
{
    ...
    for (uint32_t i = 0; i < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); i++) {

        if (abs(gs_imgs[i] - golden_gs_imgs[i]) > 0.001) {
            error_count++;
            if (error_count < 5) {
                ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, gs_imgs[i], golden_gs_imgs[i]);
            }
        }
        total++;
        // ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, gs_imgs[i], golden_gs_imgs[i]);
