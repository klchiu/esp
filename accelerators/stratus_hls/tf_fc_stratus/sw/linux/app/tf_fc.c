...
int main(int argc, char **argv)
{
    ...
    /* for(int t = 0; t < 2; t++){ */
    	    ...
    	    if(basic_hw == 0)
    		    ...
    		    printf("HW: Initialization time: %llu ns\n", init_ns);
    		    printf("HW: Basic accumulation level execution time: %llu ns\n", basic_ns);
    		    printf("HW: Basic accumulation level total time: %llu ns\n", basic_ns+init_ns);
    	    }
    /* } */
    ...
    mult_acc_buf = (token_t*)esp_alloc(mult_len * 3);
    cfg_tf_mult3[0].hw_buf = mult_acc_buf;
    ...
    add_acc_buf = (token_t*)esp_alloc(add_len * 3);
    cfg_tf_add3[0].hw_buf = add_acc_buf;
    ...
    for (int b = 0; b < batches; ++b) {
    	...
    	if(output_hw == 0){
    		...
    		load_buffer(mult_acc_buf, 0, mult_batch, 0, weights_data, mult_len);

    		esp_run_no_print(cfg_tf_mult3, NACC);

    		store_buffer(mult_acc_buf, 0, mult_batch, mult_len);
    	}
        ...
        if (bias_data) {
    		...
    		if(output_hw == 0){
    			...
    			load_buffer(add_acc_buf, output_depth * b, total_3, 0, bias_data, add_len);

    			esp_run_no_print(cfg_tf_add3, NACC);

    			store_buffer(add_acc_buf, output_depth * b, total_3, add_len);
    ...
    if(output_hw == 0)
    	    printf("SW: Output level execution time: %llu ns\n", output_ns);
    else{
    	    printf("HW: Initialization time: %llu ns\n", init_ns);
    	    printf("HW: Output level execution time: %llu ns\n", output_ns);
    	    printf("HW: Outout level total time: %llu ns\n", output_ns+init_ns);
    }
    ...
    tf_mult3_cfg_000[0].tf_src_dst_offset_1 = mult_len;
    tf_mult3_cfg_000[0].tf_src_dst_offset_2 = mult_len + mult_len;
    tf_mult3_cfg_000[0].chunk_size = 8192;

    //printf("Allocating add\n");
    ...
    tf_add3_cfg_000[0].tf_src_dst_offset_1 = add_len;
    tf_add3_cfg_000[0].tf_src_dst_offset_2 = add_len + add_len;
    tf_add3_cfg_000[0].chunk_size = 8192;
    ...
    if(batch_hw == 0){
	    ...
	    //printf("Load\n");
	    load_buffer(mult_acc_buf, 0, above_batch_weights_data, 0, mult_above_batch, mult_len);

	    //printf("Run\n");
	    esp_run_no_print(cfg_tf_mult3, NACC); // esp_run(cfg_tf_mult3, NACC);

	    //printf("Store\n");
	    store_buffer(mult_acc_buf, 0, mult_above_batch, mult_len);
	...
	if(batch_hw == 0){
		...
		load_buffer(add_acc_buf, 0, total_4, 0, above_batch_bias_data, add_len);

		esp_run_no_print(cfg_tf_add3, NACC);

		store_buffer(add_acc_buf, 0, total_4, add_len);
	}
    }
    ...
    if(batch_hw == 0)
	    ...
	    printf("HW: Initialization time: %llu ns\n", init_ns);
	    printf("HW: Batch level execution time: %llu ns\n", batch_ns);
	    printf("HW: Batch level total time: %llu ns\n", batch_ns+init_ns);
    }
