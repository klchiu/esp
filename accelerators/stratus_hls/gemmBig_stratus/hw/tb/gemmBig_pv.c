...
#include "gemmBig_pv.h"

void _gemmBig_pv(double *mtx_inA, double *mtx_inB, double *mtx_out, size_t is_trans, size_t rowsA, size_t colsA,
              size_t colsB)
{
    ...
    if (is_trans) {
        ...
        cblas_dgemmBig(CblasRowMajor, // row-major order representation
...
void gemmBig_pv(double_matrix_t *mtx_inA, double_matrix_t *mtx_inB, double_matrix_t **mtx_out)
{
    int      i;
    ...
    for (i = 0; i < batch_size; ++i) {
        ...
        _gemmBig_pv(&(mtx_inA->data[i * size_inA]), &(mtx_inB->data[i * size_inB]), &((*mtx_out)->data[i * size_out]),
