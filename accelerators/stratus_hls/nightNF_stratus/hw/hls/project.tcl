...
#
define_hls_module nightNF ../src/nightNF.cpp
...
foreach dma [list 64] {
    foreach plm_img_size [list 307200] {
        foreach max_pxl_width_log [list 4] {
            ...
            define_sim_config "BEHAV_$ext" "nightNF BEH" "tb TESTBENCH_$ext" -io_config IOCFG_$ext -argv $DEFAULT_ARGV

            foreach cfg [list FAST] {
                set cname $cfg\_$ext
                define_hls_config nightNF $cname -io_config IOCFG_$ext --clock_period=$CLOCK_PERIOD $COMMON_HLS_FLAGS -DHLS_DIRECTIVES_$cfg
                if {$TECH_IS_XILINX == 1} {
                    define_sim_config "$cname\_V" "nightNF RTL_V $cname" "tb TESTBENCH_$ext" -io_config IOCFG_$ext -argv $DEFAULT_ARGV -verilog_top_modules glbl
                } else {
                    define_sim_config "$cname\_V" "nightNF RTL_V $cname" "tb TESTBENCH_$ext" -io_config IOCFG_$ext -argv $DEFAULT_ARGV
                }
            }
	    }
    }
