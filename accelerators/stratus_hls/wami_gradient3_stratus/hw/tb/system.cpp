...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id     = GRADIENT_KERN_ID;
        batch       = 0;
        base_addr_0 = 0;                               // use this for output1
        base_addr_1 = num_col * num_row;               // use this for output2
        base_addr_2 = base_addr_1 + num_col * num_row; // use this for input
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_gradient3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_gradient3");
...
void system_t::load_memory()
{
    ...
    uint32_t mem_base_addr = base_addr_2; // 2*128*128
    ...
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        // convert float to fx
        data_fp = FPDATA(golden_gs_imgs[index]);
        // convert fx to bv
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        // conver fp to float
        grad_x[index] = (float)data_fp;
    }
    write_float_matrix_to_file(grad_x, srcRows - 2 * PAD, srcCols - 2 * PAD, "../output/fish_gradient_x_out.txt",
                               "Grad X");

    mem_base_addr = base_addr_1;
    ESP_REPORT_INFO("[dump_memory]: mem_base_addr: %d", base_addr_1);
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        sc_dt::sc_bv<FPDATA_WL> data_bv;
        FPDATA                  data_fp;
        ...
        data_fp = bv2fp<FPDATA, FPDATA_WL>(data_bv);
        // conver fp to float
        grad_y[index] = (float)data_fp;
    }
    write_float_matrix_to_file(grad_y, srcRows - 2 * PAD, srcCols - 2 * PAD, "../output/fish_gradient_y_out.txt",
                               "Grad Y");
...
int system_t::validate()
{
    ...
    for (uint32_t row = 0; row < (srcRows - 2 * PAD); row++) {
        for (uint32_t col = 0; col < (srcCols - 2 * PAD); col++) {
            ...
            if (abs(grad_x[i] - golden_grad_x[i]) > 0.001) {
                error_count++;
                if (error_count < 5) {
                    ESP_REPORT_INFO("%d: out1: %f\tgold1: %f", i, grad_x[i], golden_grad_x[i]);
                }
            }
            total++;
            // ESP_REPORT_INFO("%d: out1: %f\tgold1: %f", i, grad_x[i], golden_grad_x[i]);
    ...
    for (uint32_t row = 0; row < (srcRows - 2 * PAD); row++) {
        for (uint32_t col = 0; col < (srcCols - 2 * PAD); col++) {
            ...
            if (abs(grad_y[i] - golden_grad_y[i]) > 0.001) {
                error_count++;
                // if (error_count < 5) {
                    ESP_REPORT_INFO("%d: out2: %f\tgold2: %f", i, grad_y[i], golden_grad_y[i]);
                // }
            }
            total++;
            // ESP_REPORT_INFO("%d: out2: %f\tgold2: %f", i, grad_y[i], golden_grad_y[i]);
