...
/**
 * Baremetal device driver for NIGHTDWT
 *
...
#define SLD_NIGHTDWT 0x13
#define DEV_NAME        "sld,nightDwt_stratus"
...
#define NIGHTDWT_BUF_SIZE (ROWS * COLS * 2 * sizeof(pixel))

/* Size of the contiguous chunks for scatter/gather */
#define CHUNK_SHIFT 10
#define CHUNK_SIZE  BIT(CHUNK_SHIFT)
#define NCHUNK                                                                      \
    ((NIGHTDWT_BUF_SIZE % CHUNK_SIZE == 0) ? (NIGHTDWT_BUF_SIZE / CHUNK_SIZE) \
                                              : (NIGHTDWT_BUF_SIZE / CHUNK_SIZE) + 1)

// User defined registers
#define NIGHTDWT_NIMAGES_REG 0x40
#define NIGHTDWT_NROWS_REG   0x44
#define NIGHTDWT_NCOLS_REG   0x48
...
int main(int argc, char *argv[])
{
    ...
    ndev = probe(&espdevs, VENDOR_SLD, SLD_NIGHTDWT, DEV_NAME);
    if (ndev <= 0) {
    ...
    for (n = 0; n < ndev; n++) {
...
#else
        {
            ...
            mem = aligned_malloc(NIGHTDWT_BUF_SIZE);
    ...
    #include "data_480x640.h"
            ...
            iowrite32(dev, NIGHTDWT_NIMAGES_REG, 1);
            iowrite32(dev, NIGHTDWT_NROWS_REG, ROWS);
            iowrite32(dev, NIGHTDWT_NCOLS_REG, COLS);
#if (ROWS == 32 && COLS == 32)
            iowrite32(dev, NIGHTDWT_DO_DWT_REG, 0);
#else
            iowrite32(dev, NIGHTDWT_DO_DWT_REG, 1);
#endif
