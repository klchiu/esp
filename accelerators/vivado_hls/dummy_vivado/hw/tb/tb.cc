...
int main(int argc, char **argv)
{
    ...
    printf("**** start testinig dummy_vivado *****\n");

    /* <<--params-->> */
    const unsigned output = 8;
    const unsigned input2 = 8;
    const unsigned input1 = 8;
    ...
    in_words_adj  = round_up(input1, VALUES_PER_WORD);
    out_words_adj = round_up(input1, VALUES_PER_WORD);
    ...
    for (unsigned i = 0; i < 1; i++)
        for (unsigned j = 0; j < input1; j++)
            outbuff_gold[i * out_words_adj + j] = (word_t)(j+7);

    // Call the TOP function
    top(mem, mem,
        /* <<--args-->> */
        output, input2, input1, load, store);
