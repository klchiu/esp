...
void system_t::config_proc()
{
    ...
    // Config
    {
        ...
        kern_id      = SD_UPDATE_KERN_ID;
        batch        = 0;
        base_addr_0  = 0;                                   // use this for output
        base_addr_1  = 6;                                   // use this for input 1
        base_addr_2  = base_addr_1 + 6 * num_col * num_row; // use this for input 2
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - wami_sd_update3");
        ...
        ESP_REPORT_TIME(end_time, "END - wami_sd_update3");
...
void system_t::load_memory()
{
    ...
    for (uint32_t index = 0; index < 6 * (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        // convert float to fx
        data_fp = FPDATA(golden_I_steepest[index]);
        // convert fx to bv
    ...
    mem_base_addr = base_addr_2; // 6*128*128
    ESP_REPORT_INFO("[load_memory]: mem_base_addr: %d", mem_base_addr);
    for (uint32_t index = 0; index < (srcRows - 2 * PAD) * (srcCols - 2 * PAD); index++) {
        ...
        // convert float to fx
        data_fp = FPDATA(golden_sub[index]);
        // convert fx to bv
...
void system_t::dump_memory()
{
    ...
    for (uint32_t index = 0; index < 6; index++) {
        ...
        // conver fp to float
        sd_delta_p[index] = (float)data_fp;
    }
    write_float_matrix_to_file(sd_delta_p, 6, 1, "../output/fish_sd_update_out.txt", "SD Update");
...
int system_t::validate()
{
    ...
    for (uint32_t i = 0; i < 6; i++) {
        float error_rate = abs(sd_delta_p[i] - golden_sd_delta_p[i]) / golden_sd_delta_p[i];
        if (error_rate > 0.01) {
            error_count++;
            // ESP_REPORT_INFO("%d: out: %f\tgold: %f", i, sd_delta_p[i], golden_sd_delta_p[i]);
        }
        total++;
        // ESP_REPORT_INFO("%d: out: %f\tgold: %f\t error_rate: %f", i, sd_delta_p[i], golden_sd_delta_p[i],
        // error_rate);
    }
