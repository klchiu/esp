...
#define SLD_HUANGEMM3S128 0x02c
#define DEV_NAME "sld,huangemm3s128_stratus"
...
int main(int argc, char * argv[])
{
	...
	// 	in_words_adj = huangemm3s128_len*huangemm3s128_vec;
	// 	out_words_adj = huangemm3s128_vec;
	// } else {
	// 	in_words_adj = round_up(huangemm3s128_len*huangemm3s128_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// 	out_words_adj = round_up(huangemm3s128_vec, DMA_WORD_PER_BEAT(sizeof(token_t)));
	// }


	// in_len = in_words_adj * (huangemm3s128_n);
	// out_len = out_words_adj * (huangemm3s128_n);
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_HUANGEMM3S128, DEV_NAME);
	if (ndev == 0) {
		printf("huangemm3s128 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, HUANGEMM3S128_ROWS_REG, rows);
			iowrite32(dev, HUANGEMM3S128_COLS_REG, cols);
			iowrite32(dev, HUANGEMM3S128_LOADED_COLS_REG, loaded_cols);
