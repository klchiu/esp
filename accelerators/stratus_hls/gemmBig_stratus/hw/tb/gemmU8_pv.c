...
void _gemmU8_pv(double *mtx_inA, double *mtx_inB, double *mtx_out, size_t is_trans, size_t rowsA, size_t colsA,
              size_t colsB)
{
    ...
    if (is_trans) {
        ...
        cblas_dgemmU8(CblasRowMajor, // row-major order representation
    ...
    for (d1 = 0; d1 < rowsA; ++d1) {
		...
		int8_t inA_int8 = FPDATA8(mtx_inA[mtx_inA_i]);
		int8_t inB_int8 = FPDATA8(mtx_inB[mtx_inB_i]);
...
void gemmU8_pv(double_matrix_t *mtx_inA, double_matrix_t *mtx_inB, double_matrix_t **mtx_out)
{
    int      i;
    ...
    for (i = 0; i < batch_size; ++i) {
        ...
        _gemmU8_pv(&(mtx_inA->data[i * size_inA]), &(mtx_inB->data[i * size_inB]), &((*mtx_out)->data[i * size_out]),
