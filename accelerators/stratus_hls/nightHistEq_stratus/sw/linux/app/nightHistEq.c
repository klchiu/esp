...
#define DEVNAME "/dev/nightHistEq_stratus.0"
#define NAME    "nightHistEq_stratus"
...
static const char usage_str[] = "usage: ./nightHistEq_stratus.exe coherence infile [nimages] [ncols] [nrows] [-v]\n"
...
struct nightHistEq_test {
    struct test_info                  info;
    struct nightHistEq_stratus_access desc;
    char *                            infile;
    bool                              infile_is_raw;
    unsigned                          nimages;
    unsigned                          rows;
...
static inline struct nightHistEq_test *to_nightHistEq(struct test_info *info)
{
    return container_of(info, struct nightHistEq_test, info);
}
...
static void init_buf(struct nightHistEq_test *t)
{
...
static inline size_t nightHistEq_size(struct nightHistEq_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(int);
}

static inline size_t nightHistEq_size_opt(struct nightHistEq_test *t)
{
    return t->rows * t->cols * t->nimages * sizeof(short);
}

static void nightHistEq_alloc_buf(struct test_info *info)
{
    struct nightHistEq_test *t = to_nightHistEq(info);

    t->hbuf = malloc0_check(nightHistEq_size_opt(t));
    if (!strcmp(info->cmd, "test")) {
        t->sbuf_in  = malloc0_check(nightHistEq_size(t));
        t->sbuf_out = malloc0_check(nightHistEq_size(t));
    }
}

static void nightHistEq_alloc_contig(struct test_info *info)
{
    struct nightHistEq_test *t = to_nightHistEq(info);

    printf("HW buf size: %zu B\n", nightHistEq_size(t));
    if (contig_alloc(nightHistEq_size(t), &info->contig) == NULL)
        die_errno(__func__);
}

static void nightHistEq_init_bufs(struct test_info *info)
{
    struct nightHistEq_test *t = to_nightHistEq(info);

    init_buf(t);
    contig_copy_to(info->contig, 0, t->hbuf, nightHistEq_size_opt(t));
}

static void nightHistEq_set_access(struct test_info *info)
{
    struct nightHistEq_test *t = to_nightHistEq(info);
...
static void nightHistEq_comp(struct test_info *info)
{
    struct nightHistEq_test *t      = to_nightHistEq(info);
    int                      i      = 0;
...
static bool nightHistEq_diff_ok(struct test_info *info)
{
    struct nightHistEq_test *t         = to_nightHistEq(info);
    int                      total_err = 0;
    int                      i;

    contig_copy_from(t->hbuf, info->contig, 0, nightHistEq_size_opt(t));
...
static struct nightHistEq_test nightHistEq_test = {
    .info =
        {
            .name         = NAME,
            .devname      = DEVNAME,
            .alloc_buf    = nightHistEq_alloc_buf,
            .alloc_contig = nightHistEq_alloc_contig,
            .init_bufs    = nightHistEq_init_bufs,
            .set_access   = nightHistEq_set_access,
            .comp         = nightHistEq_comp,
            .diff_ok      = nightHistEq_diff_ok,
            .esp          = &nightHistEq_test.desc.esp,
            .cm           = NIGHTHISTEQ_STRATUS_IOC_ACCESS,
        },
};
...
{
    printf("=== Helloo from nightHistEq\n");
    if (argc < 3 || argc == 5 || argc > 7) {
        ...
        nightHistEq_test.infile = argv[2];
        printf("\tinfile: %s\n", nightHistEq_test.infile);

        if (argc == 6 || argc == 7) {
            nightHistEq_test.nimages = strtol(argv[3], NULL, 10);
            nightHistEq_test.cols    = strtol(argv[4], NULL, 10);
            nightHistEq_test.rows    = strtol(argv[5], NULL, 10);
            printf("\tnimages: %u\n", nightHistEq_test.nimages);
            printf("\tncols: %u\n", nightHistEq_test.cols);
            printf("\tnrows: %u\n", nightHistEq_test.rows);
        } else {
            nightHistEq_test.nimages = DEFAULT_NIMAGES;
            nightHistEq_test.rows    = DEFAULT_NROWS;
            nightHistEq_test.cols    = DEFAULT_NCOLS;
        }
        ...
        if (argc == 4 || argc == 7) {
            if ((strcmp(argv[3], "-v") && argc == 4) || (strcmp(argv[6], "-v") && argc == 7)) {
                ...
                nightHistEq_test.verbose = true;
                printf("\tverbose enabled\n");
            }
        }
        nightHistEq_test.nbytespp      = DEFAULT_NBYTESPP;
        nightHistEq_test.swapbytes     = DEFAULT_SWAPBYTES;
        nightHistEq_test.do_dwt        = DEFAULT_DO_DWT;
        nightHistEq_test.infile_is_raw = DEFAULT_INFILE_IS_RAW;
        nightHistEq_test.nbpp_in       = DEFAULT_NBPP_IN;
        nightHistEq_test.nbpp_out      = DEFAULT_NBPP_OUT;
        printf("\n");
    }

    return test_main(&nightHistEq_test.info, argv[1], "test");
}
